/*
  # Fix Admin Authentication and Storage Issues

  1. Password Hashing
    - Update admin user password to use proper bcrypt hashing
    - Fix authentication function to handle both plain text and hashed passwords

  2. Storage Policies
    - Fix RLS policies for storage operations
    - Ensure admin context is properly set for file uploads

  3. Schema Cache
    - Force schema refresh by updating table structure
*/
...
-- Force a schema refresh by adding a comment to the products table
COMMENT ON TABLE products IS 'Products table - updated ' || now()::text;
...
-- Grant permissions for the login function
