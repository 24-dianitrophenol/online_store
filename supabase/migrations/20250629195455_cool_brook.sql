/*
  ...
  1. Fixes
    - Remove invalid REFRESH MATERIALIZED VIEW IF EXISTS statement
    - Ensure all SQL syntax is valid
    - Keep all functionality intact

  2. Verification
    - Verify admin dashboard setup
    - Check image column exists
    - Confirm storage bucket configuration
*/
...
-- Force schema cache refresh by updating table statistics (correct syntax)
ANALYZE products;
...
DROP POLICY IF EXISTS "Allow anonymous uploads for development" ON storage.objects;
...
-- Force a complete schema refresh using valid syntax
ANALYZE;
