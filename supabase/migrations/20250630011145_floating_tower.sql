/*
  # Fix admin_audit_logs relation error

  1. Problem
    - The update_product_enhanced function is still referencing admin_audit_logs table
    - This table was dropped in previous migrations but function wasn't properly updated
    - Product updates are failing with "relation admin_audit_logs does not exist"

  2. Solution
    - Ensure all functions are properly updated to remove audit logging references
    - Recreate functions with correct signatures and no audit logging
    - Verify all related functions are working properly
*/

-- Drop any remaining problematic functions and recreate them properly
...
-- Recreate the create_product_enhanced function with proper signature
...
-- Recreate the update_product_enhanced function with proper signature
...
BEGIN
  ...
  IF create_func_exists AND update_func_exists THEN
    RAISE NOTICE 'Product functions recreated successfully without audit logging';
  ELSE
