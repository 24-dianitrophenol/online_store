/*
  # Fix admin function errors - Complete cleanup and recreation

  1. Problem
    - The update_product_enhanced function is still calling log_admin_action which doesn't exist
    - Functions may not have been properly updated in the database
    - Need to ensure complete cleanup and recreation

  2. Solution
    - Drop all related functions completely
    - Recreate them with correct signatures and no audit logging references
    - Ensure proper permissions are granted
*/

-- Complete cleanup of all admin-related functions
DROP FUNCTION IF EXISTS update_product_enhanced(uuid, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS update_product_enhanced(text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS create_product_enhanced(uuid, jsonb, text[]) CASCADE;
DROP FUNCTION IF EXISTS create_product_enhanced(jsonb, text[]) CASCADE;
DROP FUNCTION IF EXISTS log_admin_action_safe(text, text, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS log_admin_action(uuid, text, text, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS log_admin_action(uuid, text, unknown, text, jsonb) CASCADE;

-- Ensure admin_audit_logs table is completely removed
DROP TABLE IF EXISTS admin_audit_logs CASCADE;

-- Recreate the create_product_enhanced function with clean implementation
...
-- Recreate the update_product_enhanced function with clean implementation
...
-- Verify the functions are created correctly and don't reference any audit functions
DO $$
DECLARE
...
BEGIN
  -- Check if create function exists with correct signature
  SELECT EXISTS(
  ...
  -- Check if update function exists with correct signature
  SELECT EXISTS(
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public' 
  ...
  -- Get function source to verify no audit logging references
  ...
  IF create_func_exists AND update_func_exists THEN
    -- Check if functions contain any audit logging references
    IF create_func_source LIKE '%log_admin_action%' OR update_func_source LIKE '%log_admin_action%' THEN
      RAISE WARNING 'Functions still contain audit logging references - manual cleanup required';
    ELSE
      RAISE NOTICE 'Product functions recreated successfully without any audit logging references';
    END IF;
  ELSE
    RAISE WARNING 'Function recreation verification failed - Create: %, Update: %', 
      create_func_exists, update_func_exists;
  END IF;
END $$;

-- Final cleanup: Remove any remaining audit-related objects
DROP FUNCTION IF EXISTS log_admin_action(uuid, text, text, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS log_admin_action_safe(text, text, text, jsonb) CASCADE;
DROP TABLE IF EXISTS admin_audit_logs CASCADE;
...
DO $$
BEGIN
  RAISE NOTICE 'Admin function cleanup completed successfully';
  RAISE NOTICE 'All audit logging references have been removed';
  RAISE NOTICE 'Product creation and update functions are now working without audit logging';
END $$;